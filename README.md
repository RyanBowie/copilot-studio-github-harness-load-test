...
**Current state: eleven reviewed runs, published on [GitHub Pages](https://ryanbowie.github.io/copilot-studio-github-harness-load-test/).** The latest one-minute / 100-request retest stopped on a **local runner admission deadline** after **41 attempts: 26 eventual greetings, 15 generic failures and 0 pending**. The other **59 requests were never sent**. This is **15/41 failures (36.585%), not 15/100**, and the requested full 100-request denominator remains unmeasured. The original ten records and all previous campaign contexts are unchanged. The earlier 25 RPM follow-up still completed its two-minute window with 50/50 greetings. **No full hour completed and no GitHub Copilot Harness capacity ceiling is established.** All costs remain **pending**, not zero. No Standard Harness data or images are imported. This repository is an offline report, not a cloud test runner. The separate email-to-workflow-to-agent scenario is not implemented.
...
This supplement still covers only the **seven earlier native cohorts**. It excludes the new locally stopped minute retest pending separately reviewed window analysis; its peaks must not be presented as maxima across all eight native cohorts. The new run's observed totals and native duration populations are available separately.
...
An **offline-only, unregistered scheduler repair prototype** rebases the next eligible start from the previous actual dispatch plus 600 ms rather than aborting on the old absolute-slot lateness boundary. It preserves journaling and explicit safety stops, never catches up, and records longer arrival windows/lower achieved rates when delayed. It has not been wired or exercised as a new live cohort. Its simulated checks are not observations, and a future count-complete result could not be called a full 100/min minute unless its actual timing supports that claim. A fresh explicit request budget is required before any further traffic.
...
| `arrivalStatus`, `stopReason` | `full_window`, `count_complete`, `stopped` or `partial`; full requires planned fixed duration, no unoffered slots and null reason. Count completion belongs only to `count_retest`, requires exactly 100 actual starts and no unused slots/reason, and is not a full-minute claim. Partial requires `observation_cutoff` (before the fixed duration for fixed-window phases). Stopped requires a reviewed enum reason, not arbitrary text. `generic_error_threshold` belongs only to historical phases, not the count-through-errors retests. Native `authentication`/`explicit_throttle` require matching errors; a pre-dispatch `account_guard` is separate and must not invent a failed agent invocation |
