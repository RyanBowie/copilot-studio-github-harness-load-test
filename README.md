...
| Representative workloads | Separately approved knowledge/workflow scenarios with requested-operation success and latency |

Further additions require a reviewed closed-schema extension before ingestion, not arbitrary metadata or raw transcripts. This change does not run experiments, grant a new request budget, or access private ledgers. All ten original records and existing documented limits remain unchanged.
...
Percentiles use nearest rank within each population; native completion is not visible response latency or backend TTFA. Runtime sessions are unknown. The prior thirteen records and seven-cohort rolling supplement remain unchanged. No new rolling-window/history analysis, capacity ceiling, full-hour validation, quota reset or universal reliability is claimed. The new one-shot gate is disabled/consumed and its permanent lock retained; no further calls are authorized by this result.
...
No postcampaign Monitor/history result is invented or borrowed from the earlier campaigns. The public run key is `paced-minute-100-local-stop`; the longer handoff suggestion was shortened to fit the closed slug contract. The source contains no raw identifiers, transcripts or private-source fingerprints.
...
Monitor at **20:48:13Z** showed **417 sessions**, updated one hour earlier: stale precohort analytics, not this cohort's session total. The original campaign's earlier 36-session snapshot is preserved as historical evidence.
...
Monitor at **19:34:29Z** still showed **36 old sessions**, **updated two hours ago**: stale precampaign analytics, not this campaign's session total. The accepted budget was request-only.
...
Post-burst Monitor checked at **16:59:36Z** said **updated 44 minutes ago**, showing one old Teams session, seven messages. Those are **stale preburst analytics**, not burst totals. Burst runtime sessions remain unknown.
...
Configuration discrepancies: the standalone Teams-send tool is missing (the conversation channel itself works), and **Search all websites is ON**, not restricted to one nominated Learn page. The tester did not edit, save or republish that configuration. Pre-pilot Monitor and a loaded post-pilot check at 16:16Z both showed no recorded sessions, but analytics can lag; neither establishes zero runtime sessions. No numerical throttle threshold or high-volume ramp was observed. Separately reviewed documentation is scoped below, not reported as observed load.
...
The public contract is load-only. Historical load fields and session-only Monitor observations are retained exactly; test-only reconstruction checks the pre-migration report fingerprints without publishing retired metadata. The seven-cohort window evidence and its schema are unchanged.
...
- `postRunMonitor` records the stale check instant, update age and old displayed counts. The validator ensures that the check follows the burst but the reported last update predates it. These are not burst session totals.
...
The public contract retains exact phase UTC markers, campaign-relative monotonic offsets, clock/evidence attestations, nearest-rank outcome-specific timings, 80 distinct returned-conversation counts, and peak eight outstanding client calls. Matching request-start/native-duration clocks supply dispatch/completion analysis; separately sampled event-clock offsets can differ (maximum 0.0378 ms at start and 0.0145 ms at completion, strictly below the reviewed 1 ms allowance). Do not force these readings or rounded UTC markers into decimal equality. Validation checks ordered phase/loop/drain/study boundaries, separate ceilings, full rolling coverage and population partitions; raw maximization and percentiles cannot be reconstructed from public aggregates.
...
Checks cover light/dark at **320, 390 and 1440 px**, all nine report sections, chart/label overflow, sticky-navigation heading visibility, rolling-window coverage/cohort selectors, paired counts and exact offsets, callback/trigger evidence, four downloads, axe WCAG A/AA rules, keyboard skip/navigation behavior, retired-section fallback with query preservation, the dark default regardless of system theme, explicit light-mode choices, invalid-data withholding, print, reduced-motion and forced-color support. Explicitly labelled synthetic states exercise populated rendering only on the ephemeral QA server; they never go into `dist`.
...
- This report contains load evidence only. A separate email-to-workflow-to-agent scenario is future scope, not implemented here.
